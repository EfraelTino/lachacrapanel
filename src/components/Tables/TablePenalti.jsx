...
export const TablePenalti = ({ dataSearch }) => {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      formData.append("action", "seepreguntas");
  ...
  const handleSearch = () => {
    ...
    const nuevoUser = user.filter(item =>
      item.preguntas.toLowerCase().includes(dataSearch.toLowerCase())
    );
              ...
              {loading ? (
                ...
                currentUsers.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 uppercase">
                      {item.preguntas}
                    </td>
